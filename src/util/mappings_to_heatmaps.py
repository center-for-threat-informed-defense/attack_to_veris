...
def get_framework_overview_layers(veris_objects, mappings, attack_data, domain,  version):
    ...
    out_layers = [
        {
            "outfile": pathlib.Path(f"veris-overview.json"),
            "layer": create_layer(
                f"veris overview",
                f"veris heatmap overview of veris mappings, scores are the number of associated entries",
                domain,
        ...
        if len(techniques_in_family) > 0:  # don't build heatmaps with no mappings
            ...
            out_layers.append({
                ...
                "layer": create_layer(
                    ...
                    f"veris heatmap for entries in the {family_id_to_name[family_id]} "
...
def get_layers_by_property(veris_objects, mappings, attack_data, domain, x_mitre, version):
...
if __name__ == "__main__":
    ...
    layers = get_framework_overview_layers(veris_objects, mappings, attack_data, args.domain, args.version)
    for p in get_x_mitre(veris_objects):  # iterate over all custom properties as potential layer-generation material
        if p == "x_mitre_family":
            continue
        layers += get_layers_by_property(veris_objects, mappings, attack_data, args.domain, p, args.version)
    print("done")
